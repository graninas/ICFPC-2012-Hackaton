module Graphics.Gloss.UI where
...
runUI :: Viewport -> UI a -> IO ()
runUI vp@(Viewport (x1, y1) (x2, y2)) (UI initWorld toplevelWidget)
      = play display black 24 (wrap initWorld) (drawFunction . unwrap) eventFunction handleTime
      where display = (InWindow "frame" (ceiling (x2-x1), ceiling (y2-y1)) (20, 20))
